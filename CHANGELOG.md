...
### Added
- `SignedPayload.expires_at(token)` and `Signer#expires_at(token)` — return the token's expiration `Time`, or `nil` if no expiration was set. Does not verify the signature.

### Changed
- CI workflow: `actions/checkout@v5` → `@v6` to match the rest of the Ruby gems.

## [0.5.0] - 2026-04-16

### Added
- `verify` accepts an array of keys to support zero-downtime secret rotation

## [0.4.0] - 2026-04-15

### Added
- `rotate(token, old_key:, new_key:, algorithm:)` to verify a token with an old key and re-sign with a new key, preserving the original data and expiration
